...
//
- (BOOL)identify:(Mat*)onlyCellPixelRatio idx:(int*)idx rotation:(int*)rotation maxCorrectionRate:(double)maxCorrectionRate validBitIdThreshold:(float)validBitIdThreshold {
    bool retVal = self.nativePtr->identify(onlyCellPixelRatio.nativeRef, *(int*)(idx), *(int*)(rotation), maxCorrectionRate, validBitIdThreshold);
    return retVal;
}
...
//
+ (Mat*)getBitsFromByteList:(Mat*)byteList markerSize:(int)markerSize rotationId:(int)rotationId {
    cv::Ptr<cv::Mat> retVal = new cv::Mat(cv::aruco::Dictionary::getBitsFromByteList(byteList.nativeRef, markerSize, rotationId));
    return [Mat fromNativePtr:retVal];
}
...
//
- (Mat*)getMarkerBits:(int)markerId rotationId:(int)rotationId {
    cv::Ptr<cv::Mat> retVal = new cv::Mat(self.nativePtr->getMarkerBits(markerId, rotationId));
    return [Mat fromNativePtr:retVal];
}

- (Mat*)getMarkerBits:(int)markerId {
    cv::Ptr<cv::Mat> retVal = new cv::Mat(self.nativePtr->getMarkerBits(markerId));
    return [Mat fromNativePtr:retVal];
}
