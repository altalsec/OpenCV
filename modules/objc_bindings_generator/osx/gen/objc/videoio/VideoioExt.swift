...
public extension VideoCapture {
    @nonobjc convenience init(source: IStreamReader, apiPreference: Int32, params: [Int32]) {
        let paramsVector = IntVector(params)
        self.init(source: source, apiPreference: apiPreference, params: paramsVector)
    }
}
...
public extension VideoCapture {
    @nonobjc func open(source: IStreamReader, apiPreference: Int32, params: [Int32]) -> Bool {
        let paramsVector = IntVector(params)
        let ret = open(source: source, apiPreference: apiPreference, params: paramsVector)
        return ret
    }
}
